...
・Developing better heuristic methods that can achieve the privacy levels that are as close to the input ones as possible. (We feel that this task is not be as easy as it may seem. Our proposed algorithm can achieve privacy levels closer to the input ones by changing the order of attributes, for example; however, a method that does not require such a procedure like parameter-tuning would be more desirable.) (Note that in the experiments of this study, we discussed cases in which the input and achieved privacy levels were the same.) 
