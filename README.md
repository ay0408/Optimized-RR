...
However, the LP problem with variable reduction is often infeasible (cf. LP_variableReduction.ipynb). Moreover, our method is (currently) more efficient than solving the linear programming problem (cf. [Cohen, Lee, and Song, 2021](https://doi.org/10.1145/3424305) and [Brand, 2020](https://doi.org/10.1137/1.9781611975994.16)); therefore, to efficiently construct a randomized response that achieves stronger privacy guarantees than the method using the Kronecker product, our method might be more useful (and is more interesting). 

Of course, there may be cases where solving the linear programming problem would be superior, depending on the optimality, the acheved $\epsilon_i$, etc. This will be an important issue to be addressed in the future.
...
・Developing better heuristic methods that can achieve the privacy levels that are as close to the input ones as possible. (We feel that this task is not be as easy as it may seem. Our proposed algorithm can achieve privacy levels closer to the input ones by changing the order of attributes, for example; however, a method that does not require such a procedure like parameter-tuning would be more desirable.) (Note that in the experiments of this study, we considered cases in which the input and achieved privacy levels were the same.) 
